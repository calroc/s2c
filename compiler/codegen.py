...
def cgList(asts, vrs, stackEnv, sep, cont):
    if asts:
        ...
        return cgList(
            ...
            [vrs[0]] + stackEnv,
            sep,
            lambda code, sv: cont([x, sep, code], sv),
        )
    else:
...
def cgArgs(args, stackEnv):
    return cgList(
        args, interval(1, len(args)), stackEnv, "", lambda code, sv: code
    )
    ...
    if A.isPrim(ast):
        ...
        if op == "%closure":
            ...
            return [
                ...
                [
                    [" INICLO(", i, ");"]
                    for i in reversed(interval(1, n))
                ],
                " END_",
                s,
            ]
...
def compileAllLambdas():
    ...
    if not lambdaTodo:
        ...
        return [
            ...
            codeGen(
                A.astSubx(ast)[0],
                [i for i in reversed(A.lamParams(ast))],
            ),
            "\n\n",
            compileAllLambdas(),
        ]
...
def codeGenerateDebug(ast):
    ...
    with open("compiler/runtime/runtime.c") as f:
        rt = f.read()
    nrt = str.replace(rt, "//__SCHEME_CODE__", code)
